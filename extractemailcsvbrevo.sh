...
# Extraer la columna y agregar la columna "Origen"
awk -F"$DELIMITER" -v col="$COLUMN_NAME" '
    ...
    NR>1 && col_index != -1 {
        ...
        print $col_index ",Brevo"
    }
' "$CSV_FILE" >> "$OUTPUT_CSV"

echo "✅ Los emails con origen han sido guardados en: $OUTPUT_CSV"